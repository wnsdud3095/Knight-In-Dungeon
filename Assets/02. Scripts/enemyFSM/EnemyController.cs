...
public class EnemyController : MonoBehaviour
{
    ...
    {
        GetComponent<EnemyFSM>().Slow(0.5f);    // 슬로우
        yield return new WaitForSeconds(4f);    // 4초 유지
        GetComponent<EnemyFSM>().ClearSlow();   // 슬로우 해제
    }
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        // if (collision.gameObject.CompareTag("Player"))
        // {
        //     GetComponent<EnemyController>()?.Freeze(2f);
        // }
    }
    ...
    {
        GetComponent<EnemyFSM>().currentMoveSpeed = GetComponent<EnemyFSM>().GetEnemyData().moveSpeed * SlowValue;
    }

    public void ClearSlow()
    {
        GetComponent<EnemyFSM>().currentMoveSpeed = GetComponent<EnemyFSM>().GetEnemyData().moveSpeed;
    }
    ...
    {
        // 빙결
        GetComponent<EnemyFSM>().currentMoveSpeed = 0f;
